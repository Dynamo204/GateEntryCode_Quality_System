...
export default function Outward() {
  ...
  // Removed series field as per new requirement
  ...
  const handleChange = (e) => {
    ...
    // Clear error if TareWeight is being changed
    if (name === 'TareWeight') setError(null);
  };

  // Fetch tare weight from weighbridge (same as QRScanner)
  const handleGetTareWeight = async () => {
    setTareWeightLoading(true);
    setError(null);
    try {
      // Use the same API as QRScanner
      ...
      // Parse weight (same as QRScanner)
      const cleaned = String(rawWeight || '').replace(/[\u0000-\u001F\u007F]/g, ' ').replace(/\s+/g, ' ').trim();
      const numberMatch = cleaned.match(/-?\d+(?:\.\d+)?/);
      const parsedWeight = numberMatch ? numberMatch[0] : '';
      if (!parsedWeight) throw new Error('Unable to parse tare weight from weighbridge response');
  ...
  const applyInitialRegData = (regData) => {
    ...
    setHeader(prev => ({
      ...
      Plant: regData.Plant || prev.Plant,
  //    BalanceQty: regData.ExpectedQty || prev.BalanceQty,
      SAP_Description: regData.SAP_Description || prev.Remarks,
  ...
  }, [header.GateEntryDate, currentYear]);
  ...
  // Set InwardTime as auto-filled current time in UI on mount and when mode changes
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate TareWeight: must not be empty, null, or zero
    const tare = header.TareWeight;
    if (tare === undefined || tare === null || String(tare).trim() === "" || Number(tare) === 0) {
      setError("Tare Weight is required and must be greater than zero");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Do not generate GateEntryNumber here; let backend handle it
      updatedHeader.GateEntryNumber = "";

      const payload = transformDataForAPI({
        ...updatedHeader,
        Vendor: updatedHeader.VendorITP,
        Material: updatedHeader.MaterialITP,
        GateEntryDate: `${updatedHeader.GateEntryDate}T00:00:00`,
        InwardTime: hhmmssToSapDuration(updatedHeader.InwardTime || nowTime()),
      ...
      <form onSubmit={handleSubmit} onKeyDown={handleKeyDown}>
        <section className="form-section">
          ...
          <div className="grid-3-cols">
            ...
            {/* Series field removed as per new requirement */}
            ...
            <div className="form-group lookup-field" style={{ position: 'relative' }}>
              ...
              <input
                ...
                style={fieldsReadOnly ? { background: '#f0f0f0' } : {}}
              />
