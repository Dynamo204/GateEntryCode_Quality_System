...
export default function GateEntryOutwardCompletion() {
  ...
  const createInitialHeaderState = () => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      initialState[`PurchaseOrderItem${s}`] = "";
  ...
  // Auto-update OutwardTime live when enabled
  useEffect(() => {
    if (outwardTimeLive) {
  ...
  // Initialize OutwardTime on mount
  useEffect(() => {
    setHeader((prev) => ({ ...prev, OutwardTime: hhmmssNow() }));
  ...
  const handleLoad = async (e) => {
    ...
    try {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        if (hydrated[`PurchaseOrderNumber${suffix}`]) {
          poCount = i;
        }
      }
      ...
      // Always enable live mode for OutwardTime until saved
      setOutwardTimeLive(true);
      
      setHeader({
        ...hydrated,
        GateEntryDate: hydrated.GateEntryDate || nowDate,
        GateOutDate: nowDate,
        InwardTime: hydrated.InwardTime || '',
        OutwardTime: nowTime
      });
    } catch (err) {
      setError(
  ...
  const handleSaveOutward = async (e) => {
    ...
    try {
      ...
      // PATCH by GateEntryNumber (business key)
      const resp = await api.patch(`/headers/${header.GateEntryNumber}`, payload);
            ...
            <div key={idx} className="po-entry-card">
              ...
              <div className="grid-4-cols">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" name={`PurchaseOrderItem${suffix}`} value={header[`PurchaseOrderItem${suffix}`]} readOnly />
                </div>
                <div className="form-group">
