...
import { createHeader, fetchNextGateNumber, fetchInitialRegistrations, fetchSalesOrderByNumber,fetchSupplierCustomerDetails,fetchSalesOrderDetails } from "../../api";
...
export default function Outward() {
  ...
  useEffect(() => {
    const lookupInitialReg = async () => {
      ...
      try {
        ...
        // Filter to only show Success status registrations (correct field: Status_Text)
        const successResults = results.filter(r => r.Status === "01" || r.Status_Text === "Success");
  ...
  // Fetch sales order details and update header fields
  const fetchSalesOrderByNumber = async (salesOrderNumber) => {
    console.log("Fetching sales order details for:", salesOrderNumber); // Debug log
    try {
      const response = await fetchSalesOrderDetails(salesOrderNumber);
      const data = response.data;
      console.log("Sales order API response:", data); // Debug log
      // Get the first item from the to_Item.results array
      const item = data?.to_Item?.results?.[0];
      if (item) {
        setHeader(prev => ({
          ...prev,
...
// Helper: Get CustomerName by Customer number from customer list
function getCustomerNameByNumber(customers, customerNumber) {
  const found = customers.find(c => c.Customer === customerNumber);
  ...
  // Auto-fill CustomerName when Customer number changes
  useEffect(() => {
    if (header.Customer && allCustomers.length > 0) {
      const customerName = getCustomerNameByNumber(allCustomers, header.Customer);
      if (customerName && header.CustomerName !== customerName) {
        setHeader(prev => ({ ...prev, CustomerName: customerName }));
      }
    }
  }, [header.Customer, allCustomers]);
  // Auto-fetch sales order details when SalesDocument changes
  useEffect(() => {
    if (header.SalesDocument) {
      fetchSalesOrderByNumber(header.SalesDocument);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = transformDataForAPI({
        ...header,
        SeriesCode: series, // Always include SeriesCode for backend prefix
      ...
      setHeader(prev => ({ ...prev, GateEntryNumber: createdGateNumber }));
      setResult({
        success: true,
      ...
      <form onSubmit={e => e.preventDefault()} onKeyDown={handleKeyDown}>
        <section className="form-section">
          ...
          <div className="grid-3-cols">
            ...
            <div className="form-group lookup-field" style={{ position: 'relative', width: '100%', background: '#e8f4ff', border: '1.5px solid #4a90d9', borderRadius: '8px', padding: '10px 12px' }}>
              <label className="form-label" style={{ color: '#1a5fa8', fontWeight: '700' }}>Registration Number</label>
              <input
                ...
                required
                autoComplete="off"
                style={{ width: '100%', background: '#ffffff', border: '1.5px solid #4a90d9', fontWeight: '600' }}
              />
        ...
        <section className="form-section">
          ...
          <div className="sd-entry-card">
            <div className="grid-2-cols">
              ...
              <div className="form-group">
                <label className="form-label required">Registered Quantity</label>
                <input
                  className="form-input"
                  name="BalanceQty"
                  value={header.BalanceQty}
                  onChange={handleChange}
                  placeholder="Registered Quantity"
                  required
                />
              </div>
            </div>
