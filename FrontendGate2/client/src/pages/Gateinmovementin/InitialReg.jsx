...
import { initialRegistration, fetchInitialRegistrations, updateInitialRegistration, fetchSalesOrderSuggestions, checkVehicleStatus, transporterDetails, sendinitialusermail } from "../../api";
...
export default function InitialRegistration() {
  ...
  // Transporter autocomplete effect
  useEffect(() => {
    const val = (formData.Transporter || "").trim();
    if (val.length < 2) {
    ...
    const fetchTransporters = async () => {
      try {
        ...
        if (!ignore) {
          const suggestions = res.data?.results || res.data?.d?.results || res.data || [];
          setTransporterSuggestions(suggestions);
            ...
            try {
              ...
              const usedQty = regResults
                .filter(r => 
                  ...
                  (r.Status === "01" || !r.Status) // Only count successful ones
                )
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          {s.UsedQty !== undefined && <span style={{marginLeft:8}}>Dispatch: {s.UsedQty}</span>}
                          {s.TotalQty !== undefined && <span style={{marginLeft:8}}>Order: {s.TotalQty}</span>}
                        </div>
                      </li>
                    );
                  })}
              ...
              {showTransporterSuggestions && transporterSuggestions.length > 0 && (
                <ul className="so-suggestion-list" style={{ zIndex: 10, maxHeight: 200, overflowY: 'auto' }}>
                  {transporterSuggestions.map((t, i) => (
                    <li
                      key={t.TransporterCode + '_' + i}
                      onClick={() => {
                        setFormData(f => ({
                          ...f,
                          Transporter: t.TransporterName || t.TransporterCode || "",
                    ...
                    >
                      <div>
                        <strong>{t.TransporterCode}</strong>
