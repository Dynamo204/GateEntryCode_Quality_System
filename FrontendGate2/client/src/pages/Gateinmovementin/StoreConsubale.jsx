...
export default function StoresConsumable() {
  ...
  // Fetch next SC Gate Entry Number on mount or when starting a new entry
  useEffect(() => {
    const fetchNextGateEntryNumber = async () => {
      try {
        const resp = await fetch("/api/next-gatenumber-SC");
        const data = await resp.json();
        if (data.next) {
          setHeader(prev => ({ ...prev, GateEntryNumber: data.next }));
        }
      } catch (err) {
  ...
  const generatePdfBlob = async (data) => {
    ...
    const headerRows = [
      ...
      ["Vendor Name:", getField('vendorName'), "Vehicle No:", getField('vehicle')],
      ["Mode of Transport:", getField('transporterMode'), "Transporter Name:", getField('transporter')],
  ...
  // Print and auto-download PDF slip
  const printPdfSlip = async (data) => {
    const pdfBlob = await generatePdfBlob(data);
    // Auto-download the PDF
    const fileName = `StoresConsumable_${data.gateEntry || 'Slip'}.pdf`;
  ...
  const submit = async () => {
    ...
    try {
      ...
      const successPayload = {
        ...
        po: Array.from(new Set(usedItems.map(i => i.PurchaseOrderNumber).filter(Boolean))).join(', '),
        vendorName: Array.from(new Set(usedItems.map(i => i.VendorName).filter(Boolean))).join(', '),
      ...
      // Auto-download slip after entry creation
      ...
      setMsg("");
      setMsgType("");
      // Auto-download the slip PDF
      setTimeout(() => {
        printPdfSlip(successPayload).catch(() => {}); // Ignore print errors here, just download
      }, 500);
    } catch (err) {
